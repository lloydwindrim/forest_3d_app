...
This script is very similar to hovermap_pipeline_las.py, except that the stem_model.predict argument dist_threshold=float("inf")
so that all of the input points are segmented so that the intensity can be paired with the segmented pointcloud. The
individual segmented pointclouds are each output as a las file with stem label and intensity instead of an asc file.
'''
...
seg_list = stem_model.predict(xyzr_list,segmenter_addr,batchsize=20,dist_threshold=float("inf"))
